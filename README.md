...
- JISOO : EduCare 시험 감독 시스템을 만들면서 처음 설계부터 DB 구축, 백엔드 구성, 프론트엔드 구성까지 하나하나 동료들과 협업해 나가면서 만들어가는 과정에서 제 실력을 업그레이드 할 수 있는 시간들이었습니다. 또한 에러를 마주하더라도 당황하지 않고 구글링하여 고쳐 나가는 방법에 익숙해지게 되었습니다. 각자가 스스로의 파트를 다들 끈기있고 책임감 있게 해 준 덕에 저희가 처음 설계한 대로 웹 구현이 마무리 된 것 같습니다. 모두들 고생하셨어요!

- JEONGMIN : 기존의 mvc 패턴을 적용하는 서버들과 다른 경량 웹서비인 flask를 배울 수 있었던 점이 흥미로웠습니다. 또한, ML 모델을 flask에 띄우고, 원하는 기능들을 추가하면서 실력을 향상시킬 수 있었습니다. 각 파트별로 다들 충실히 잘 해준덕에 원하는 웹 어플리케이션을 구현할 수 있었던 것 같습니다. 

- JONGWOOK : Spring Boot, Flask 백엔드 서버와 MYSQL 데이터베이스 연동을 해본 경험은 좋은 경험이었고, YOLOv3 객체 탐지 모델 결과물을 웹 페이지에 띄우는 작업에 참여할 수 있어서 좋았습니다. 추가로, 훌륭한 팀원들과 함께 협업하면서 개발을 하게 되어서 좋았고 향후 추억에 남을 값진 시간이었던 것 같습니다.  
